  ...
  return (
    <div className={`${modal.show === true ? "block" : "hidden"}`}>
      ...
      >
        <div
          ref={modalContentRef}
          className={`max-h-[480px] w-full ${modal.width ? modal.width : "max-w-sm"} overflow-auto rounded-lg bg-white shadow-lg`}>
          <div className="flex items-center justify-between p-3 border-b lg:p-6">
            <h1 className="font-poppins text-[18px] font-semibold text-[#073535f3]">
              {modal.title}
            </h1>
            <button onClick={() => {
              dispatch(showModal({ show: false, title: null, width: null, selectedItem: null, }));
